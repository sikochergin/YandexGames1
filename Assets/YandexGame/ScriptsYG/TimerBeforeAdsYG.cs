...
public class TimerBeforeAdsYG : MonoBehaviour
{
    ...
    [SerializeField] private SoundVol soundVol;
    ...
    IEnumerator CheckTimerAd()
    {
        ...
        if (!_startMenu.Starter) 
        {
            if (!soundVol.muted)
            {
                soundVol.IsSound = true;
                soundVol.GetComponent<AudioSource>().volume = 0.1f;
            }
    ...
    {
        soundVol.IsSound = false;
        soundVol.GetComponent<AudioSource>().volume = 0f;
        pause.IsPause = true;
        bool process = true;
        while (process)
        {
